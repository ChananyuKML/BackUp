...
GROUND_TRUTH_DIR := $(DATA_DIR)/$(MODEL_NAME)-ground-truth
...
ifeq ($(LANG_TYPE),Indic)
	...
	GENERATE_BOX_SCRIPT =src/python/generate_wordstr_box.py
else
ifeq ($(LANG_TYPE),RTL)
	NORM_MODE =3
	...
	NORM_MODE =2
	RECODER=
	GENERATE_BOX_SCRIPT =src/python/generate_line_box.py
endif
endif
...
$(ALL_LSTMF): $(ALL_FILES:%.gt.txt=%.lstmf)
	...
	$(PY_CMD) src/python/shuffle.py $(RANDOM_SEED) "$@"
